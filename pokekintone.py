...
def post_comment_to_record(app_id, rec_id, comment_text):
    method = 'POST'
    path = '/k/v1/record/comment.json'
    comment = {"text":comment_text}
    body = {"app":app_id, "record": rec_id, "comment":comment}
    kintone_request(method, path, body)

    method = 'PUT'
    path = '/k/v1/record.json'
    datetime_now = datetime.now() + timedelta(hours=-9)
    record = {"Comment_Updated_Datetime":{"value": datetime_now.isoformat()}}
    body = {"app":app_id, "id": rec_id, "record":record}
    return kintone_request(method, path, body)

def post_comment_to_thread(space_id, thread_id, comment_text):
    method = 'POST'
    path = '/k/v1/space/thread/comment.json'
    comment = {"text":comment_text}
    body = {"space":space_id, "thread": thread_id, "comment":comment}
    return kintone_request(method, path, body)
...
def get_args():
    ...
    parser.add_argument('-kd', '--kintone-domain', help='kintone domain \{subdomain\}.cybozu.com', required=True)
    parser.add_argument('-ku', '--kintone-user', help='kintone user name', required=True)
    parser.add_argument('-kp', '--kintone-password', help='kintone password', required=True)
    parser.add_argument('-kU', '--kintone-basic-user', help='kintone Basic authentication user name', required=False)
    parser.add_argument('-kP', '--kintone-basic-password', help='kintone Basic authentication password', required=False)
    parser.add_argument('-ai', '--kintone-app-id', help='kintone app. ID.', required=True)
    parser.add_argument('-si', '--kintone-space-id', help='kintone space ID.', required=False)
    parser.add_argument('-ti', '--kintone-thread-id', help='kintone thread ID.', required=False)
    parser.add_argument('-gi', '--kintone-guest-space-id', help='kintone guest space ID.', required=False)
...
def build_kintone_params(args):
    global global_kintone_domain
    global_kintone_domain = str(args.kintone_domain)

    global global_kintone_auth_token
    global_kintone_auth_token = base64.b64encode(str(args.kintone_user) + ':' + str(args.kintone_password))

    global global_kintone_basic_token
    if args.kintone_basic_user and args.kintone_basic_password:
        global_kintone_basic_token = base64.b64encode(str(args.kintone_basic_user) + ':' + str(args.kintone_basic_password))
    else:
        global_kintone_basic_token = ''

    global global_kintone_app_id
    global_kintone_app_id = str(args.kintone_app_id)

    global global_kintone_space_id
    global global_kintone_thread_id
    if args.kintone_space_id and args.kintone_thread_id:
        global_kintone_space_id = args.kintone_space_id
        global_kintone_thread_id = args.kintone_thread_id
    else:
        global_kintone_space_id = None
        global_kintone_thread_id = None

    return global_kintone_domain, global_kintone_auth_token, global_kintone_basic_token, global_kintone_app_id, global_kintone_space_id, global_kintone_thread_id

def kintone_request(method, path, query_or_body):
    query = ''
    body = ''
    if method is 'POST' or method is 'PUT':
        body = json.dumps(query_or_body)
    else:
        query = query_or_body

    headers = {}
    headers['X-Cybozu-Authorization'] = global_kintone_auth_token
    headers['Authorization'] = 'Basic ' + global_kintone_basic_token
    if method is 'POST' or method is 'PUT':
        headers['Content-Type'] = 'application/json'
    connect = httplib.HTTPSConnection(global_kintone_domain + ":443")
    connect.request(method, path + query, body, headers)
...
def search_record(app_id, pokename):
    method = 'GET'
    path = '/k/v1/records.json'
    query = "&query=" + urllib2.quote("English_Name=\"" + pokename + "\"") + "&totalCount=true"
    query = "?app=" + app_id + query
    res = kintone_request(method, path, query)
...
transform_from_wgs_to_gcj(Location(Fort.Latitude, Fort.Longitude))
    ...
    for poke in visible:
        ...
        if distance < max_distance:
            ...
            # post comment to kintone
            record = search_record(global_kintone_app_id, pokename)
            if record:
                rec_id = record['$id']['value']
                comment_text = 'I\'m just ' + \
                ...
                post_comment_to_record(global_kintone_app_id, rec_id, comment_text)
                if global_kintone_space_id and global_kintone_thread_id and record['Rare_Pokemon']['value']:
                    comment_text = pokename + ': ' + comment_text
                    post_comment_to_thread(global_kintone_space_id, global_kintone_thread_id, comment_text)
                print('[+] kintone operation called ({})'.format(pokename))
